...
if __name__ == '__main__':
    def browsefunc():
        filename = tkFileDialog.askopenfilename(filetypes=(("wav files", "*.wav"), ("All files", "*.*")))
        ent1.insert(ttk.END, filename)  # add this
    ...
    def convertvoice():
        ...
        in_fpath = folderPath.get()
        print(in_fpath)
        in_fpath = Path(in_fpath.replace("\"", "").replace("\'", ""))
    ...
    gui.title("Voice Converter")
    
    folderPath = StringVar()
    script = StringVar()
    
    Label(gui, text="Expected Voice Wav File").grid(row=0, column=2)
    
    ent1 = ttk.Entry(gui, textvariable=folderPath, font=40)
    ent1.grid(row=2, column=2)

    b1 = ttk.Button(gui, text="browse", font=40, command=browsefunc)
    b1.grid(row=2, column=4)

    c = ttk.Button(gui, text="Connect and Synthesize", command=convertvoice)
    c.grid(row=7, column=2)
    
    
#    c1 = ttk.Button(gui, text="convert", command=convert)
